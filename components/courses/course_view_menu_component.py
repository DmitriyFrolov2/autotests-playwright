...
class CourseViewMenuComponent(BaseComponent):
    ...
    @allure.step('Открываем меню курса с индексом "{index}" и нажимаем «Редактировать»')
    def click_edit(self, index: int):
        self.menu_button.click(nth=index)

        self.edit_menu_item.check_visible(nht=index)
        self.edit_menu_item.click(nth=index)

    @allure.step('Открываем меню курса с индексом "{index}" и нажимаем «Удалить»')
    def click_delete(self, index: int):
        self.delete_menu_item.click(nth=index)

        self.delete_menu_item.check_visible(nht=index)
        self.delete_menu_item.click(nth=index)